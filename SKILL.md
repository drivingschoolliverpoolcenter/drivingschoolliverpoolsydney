...
You are about to reverse-engineer and rebuild **$ARGUMENTS** as pixel-perfect static clones.

Output is **static files only**: semantic HTML5, plain CSS with custom properties, and minimal vanilla JavaScript. No React, Vue, Angular, Bootstrap, Tailwind, jQuery, build tools, or heavy animation libraries.
...
All generated files must meet these requirements:

- **HTML:** Semantic HTML5 — use `<header>`, `<nav>`, `<main>`, `<section>`, `<article>`, `<aside>`, `<footer>` correctly
- **CSS:** Plain CSS with CSS custom properties (`--color-*`, `--space-*`, `--font-*`), reusable utility classes, mobile-first responsive layout, no inline styles unless strongly justified
- **JS:** Minimal vanilla JavaScript only where behaviour cannot be achieved with CSS alone. No libraries.
- **Images:** Lightweight handling — download originals, serve from `images/`, use correct `width`/`height` attributes, `loading="lazy"` on below-fold images
- **No unused CSS or JS.** No commented-out blocks.
- **No build tools** unless absolutely required (e.g., a single-purpose asset download script is fine; a bundler or transpiler is not).
...
├── index.html
├── css/
│   ├── reset.css          ← base reset / box-sizing
│   ├── variables.css      ← all CSS custom properties (colors, fonts, spacing, radii, shadows)
│   ├── utilities.css      ← reusable utility classes (.container, .sr-only, .visually-hidden, etc.)
│   └── main.css           ← section-by-section styles, imports reset + variables + utilities
├── js/
│   └── main.js            ← all vanilla JS (only if needed)
├── images/                ← all downloaded images, favicons, OG images
...
│   └── design-references/ ← screenshots at desktop + mobile widths
└── scripts/
    └── download-assets.mjs
```
...
3. Create the output directories if they don't exist: `css/`, `js/`, `images/`, `fonts/`, `docs/research/`, `docs/research/components/`, `docs/design-references/`, `scripts/`. For multiple clones, also prepare per-site folders like `docs/research/<hostname>/` and `docs/design-references/<hostname>/`.
...
- **Short, precise batches.** Do one extraction, write one spec, dispatch one agent — then move to the next. Never batch more than you can verify.
- **Don't re-read files you already have in context.** If you extracted styles 30 seconds ago, use those values — don't re-run the extraction script for the same element.
- **Don't analyse irrelevant files.** If a section has no JS interaction, don't inspect `main.js`. If a builder needs only one image, don't enumerate all 40 in `images/`.
- **Check local folders first.** Before downloading any asset, list `images/` to see what's already there.
...
Extract the actual text, images, videos, and SVGs from the live site. This is a clone, not a mockup. Use `element.textContent`, download every `<img>` and `<video>`, extract inline `<svg>` elements verbatim. The only time you generate content is when something is clearly server-generated and unique per session.

**Layered assets matter.** A section that looks like one image is often multiple layers — a background watercolor/gradient, a foreground UI mockup PNG, an overlay icon. Inspect each container's full DOM tree and enumerate ALL `<img>` elements and background images within it, including absolutely-positioned overlays. Missing an overlay image makes the clone look empty even if the background is correct.

### 4. Foundation First

Nothing can be built until the foundation exists: `css/variables.css` with all design tokens (colors, fonts, spacing), `css/reset.css`, `css/utilities.css`, and all global assets (fonts, favicons, images). This is sequential and non-negotiable. Everything after this can be parallel.
...
2. If they do, it's scroll-driven. Extract the mechanism: `IntersectionObserver`, `scroll-snap`, `position: sticky`, CSS `animation-timeline`, or JS scroll listeners.
3. If nothing changes on scroll, THEN click/hover to test for click/hover-driven interactivity.
4. Document the interaction model explicitly in the component spec: "INTERACTION MODEL: scroll-driven with IntersectionObserver" or "INTERACTION MODEL: click-to-switch with CSS class toggle."
...
Every section gets a specification file in `docs/research/components/` BEFORE any builder is dispatched. This file is the contract between your extraction work and the builder agent. The builder receives the spec file contents inline in its prompt — the file also persists as an auditable artifact.

The spec file is not optional. It is not a nice-to-have. If you dispatch a builder without first writing a spec file, the builder will guess to fill gaps.

### 9. Files Must Be Valid

Every builder agent must verify their HTML is valid and all CSS references exist before finishing. After merging worktrees, you verify `index.html` opens in a browser without console errors. A broken file is never acceptable, even temporarily.
...
**Fonts** — Inspect `<link>` tags for Google Fonts or self-hosted fonts. Check computed `font-family` on key elements (headings, body, code, labels). Document every family, weight, and style actually used. Add Google Fonts via `<link>` in `index.html`, or download and self-host in `fonts/` with `@font-face` in `css/variables.css`.

**Colors** — Extract the site's color palette from computed styles across the page. Write all colors as CSS custom properties in `css/variables.css`: `--color-primary`, `--color-bg`, `--color-text`, etc. Add semantic aliases where useful.

**Spacing & sizing** — Extract recurring spacing values (padding, margin, gap) and map them to custom properties: `--space-xs: 8px`, `--space-sm: 16px`, `--space-md: 24px`, `--space-lg: 48px`, `--space-xl: 80px`, etc.

**Typography scale** — Extract font-size, font-weight, line-height, letter-spacing for each heading level and body text. Map to custom properties: `--text-xs`, `--text-sm`, `--text-base`, `--text-lg`, `--text-xl`, `--text-2xl`, etc.

**Favicons & Meta** — Download favicons, apple-touch-icons, OG images, webmanifest to `images/`. Update `index.html` `<head>` meta tags.

**Global UI patterns** — Identify any site-wide CSS: custom scrollbar styling, scroll-snap on the page container, global keyframe animations, backdrop filters, gradients used as overlays. Add these to `css/main.css`.
...
This is sequential. Do it yourself (not delegated to an agent) since it touches shared files:

1. **Create `css/reset.css`** — box-sizing, margin/padding reset, consistent base styles
2. **Create `css/variables.css`** — all CSS custom properties: colors, fonts, spacing scale, border-radii, shadows, transitions
3. **Create `css/utilities.css`** — reusable classes: `.container`, `.sr-only`, `.visually-hidden`, `.flex`, `.grid`, common layout helpers
4. **Create `css/main.css`** — imports the above three via `@import`, plus section-by-section styles (populated by builders)
5. **Create `js/main.js`** — empty file or minimal scaffold (scroll listener for header, etc.) — only if the page needs JS
6. **Create `index.html`** — base HTML shell with correct `<head>` (charset, viewport, font links, CSS links, meta), empty `<body>` with `<header>`, `<main>`, `<footer>` placeholders
7. **Download global assets** — write and run a Node.js script (`scripts/download-assets.mjs`) that downloads all images, videos, fonts, and other binary assets from the page to the correct local folders. Use batched parallel downloads (4 at a time) with proper error handling.
8. Verify: open `index.html` in browser, no 404s in the console for CSS/font links
...
5. **Identify assets** this section uses — first check what already exists in the local `images/` folder before downloading anything. Use the browser MCP to find remaining images/videos not yet downloaded. Check for **layered images** (multiple `<img>` or background-images stacked in the same container).

6. **Assess complexity** — how many distinct sub-components does this section contain? A distinct sub-component is an element with its own unique styling, structure, and behavior (e.g., a card, a nav item, a dropdown panel).

### Step 2: Write the Component Spec File

For each section (or sub-component, if you're breaking it up), create a spec file in `docs/research/components/`. This is NOT optional.

**File path:** `docs/research/components/<section-name>.spec.md`
...
## Overview
- **Target HTML:** section in `index.html` with id `<section-id>`
- **Target CSS:** `css/main.css` — section `/* === <SectionName> === */`
- **Screenshot:** `docs/design-references/<screenshot-name>.png`
- **Interaction model:** <static | click-driven | scroll-driven | time-driven>

## HTML Structure
<Describe the semantic element hierarchy — what tag wraps what, which landmarks to use>
...
List which variables from `css/variables.css` apply here. If a new variable is needed, note it.

## States & Behaviors

### <Behavior name, e.g., "Scroll-triggered floating header">
- **Trigger:** <exact mechanism — scroll position 50px, IntersectionObserver, click on .tab-button, hover>
- **State A (before):** max-width: 100vw; box-shadow: none; border-radius: 0
- **State B (after):** max-width: 1200px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); border-radius: 16px
- **Transition:** transition: all 0.3s ease
- **Implementation:** <CSS :hover | JS scroll listener + class toggle | IntersectionObserver + class toggle | CSS animation-timeline>
...
## Assets
- Background image: `images/<file>.webp`
- Overlay image: `images/<file>.png`
- Inline SVG: (paste verbatim)
...
- Which CSS custom properties to use from `css/variables.css`
- The target HTML: which `<section id="...">` block to write in `index.html`, and which CSS block to add to `css/main.css`
- Output rules: semantic HTML5, plain CSS, no frameworks, no inline styles, `loading="lazy"` on below-fold images
- For responsive behavior: the specific breakpoint values and what changes at each
- Instruction to verify there are no broken image paths or missing CSS variable references before finishing
...
- Resolve any conflicts in `index.html` (HTML is sequential — insert sections in page-topology order) and `css/main.css` (append section CSS blocks in order)
- After each merge, verify the build still works: open `index.html` in browser, check for console errors
- If a merge introduces broken references, fix them immediately

The extract → spec → dispatch → merge cycle continues until all sections are built.

## Phase 4: Page Assembly

After all sections are built and merged, do a final pass on `index.html` and `css/main.css`:

- Ensure sections appear in correct page-topology order in `index.html`
- Ensure CSS section blocks are in a logical order in `main.css`
- Implement page-level behaviors: scroll snap on `<html>`/`<body>`, sticky `<header>`, `IntersectionObserver` for scroll-triggered animations — all in `js/main.js`
- Add `loading="lazy"` to any below-fold `<img>` tags missed by builders
- Verify: open `index.html` in browser, scroll through the entire page, no visual glitches, no console errors
...
4. For each discrepancy found:
   ...
   - If the spec was wrong: re-extract from browser MCP, update the spec, fix the CSS
   - If the spec was right but the builder got it wrong: fix the CSS to match the spec
5. Test all interactive behaviors: scroll through the page, click every button/tab, hover over interactive elements
6. Verify header transitions work, tab switching works, animations play correctly
...
- **Don't use any framework or library.** No React, Vue, Bootstrap, Tailwind, jQuery. Plain HTML, CSS, and vanilla JS only.
- **Don't use inline styles** unless there is no CSS alternative (e.g., a dynamically computed value set via JS).
- **Don't add unused CSS or JS.** Every rule must be referenced. Every JS function must be called.
...
- **Don't approximate CSS values.** "It looks like 16px" is wrong if the computed value is 18px. Extract exact values.
- **Don't skip the `width` and `height` attributes on `<img>` tags.** They prevent layout shift and are required for all images.
- **Don't reference docs from builder prompts.** Each builder gets the CSS spec inline in its prompt — never "see variables.css for colors." The builder should have zero need to read external docs.
- **Don't skip asset extraction.** Without real images and fonts, the clone will always look fake regardless of how perfect the CSS is.
...
- Total CSS custom properties defined
- Total spec files written (should match sections)
- Total assets downloaded (images, videos, fonts)
- Final file sizes (HTML, CSS, JS)
